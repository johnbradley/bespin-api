...
USER_CRED_ID=$(curl -s -H "$AUTH_HEADER" -H "$JSON_CONTENT_HEADER" -H "$ACCEPT_HEADER" -X POST -d "$PAYLOAD" $BASE_URL/dds-user-credentials/ | sed -e 's/..id..//' -e 's/,.*//')
...
curl -s -H "$AUTH_HEADER" -H "$JSON_CONTENT_HEADER" -H "$ACCEPT_HEADER" -X POST -d "$PAYLOAD" $BASE_URL/job-output-projects/

echo "Create job input file"
PAYLOAD="{\"job\":$JOB_ID, \"file_type\":\"dds_file\", \"workflow_name\":\"model\"}"
JOB_INPUT_ID=$(curl -s -H "$AUTH_HEADER" -H "$JSON_CONTENT_HEADER" -H "$ACCEPT_HEADER" -X POST -d "$PAYLOAD" $BASE_URL/job-input-files/ | sed -e 's/..id..//' -e 's/,.*//')
...
curl -s -H "$AUTH_HEADER" -H "$JSON_CONTENT_HEADER" -H "$ACCEPT_HEADER" -X POST -d "$PAYLOAD" $BASE_URL/dds-job-input-files/

echo "Create url job input file"
PAYLOAD="{\"job_input_file\":$JOB_INPUT_ID, \"url\":\"http://stuff\", \"destination_path\":\"file2.txt\", \"index\":1}"
curl -s -H "$AUTH_HEADER" -H "$JSON_CONTENT_HEADER" -H "$ACCEPT_HEADER" -X POST -d "$PAYLOAD" $BASE_URL/url-job-input-files/
